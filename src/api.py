...
app = FastAPI(title="Epic Quiz API", version="1.0.0")
...
def get_questions(
    ...
    questions = db.get_approved_questions(difficulty=difficulty, chapter=chapter)

    if story_phase:
        questions = [q for q in questions if q.get("story_phase") == story_phase]

    questions = questions[:limit]
    logger.info("GET /questions → %d results (difficulty=%r, phase=%r)", len(questions), difficulty, story_phase)
...
def get_daily_insight():
    ...
    if not candidates:
        # Fall back to highest engagement score
    ...
    # Rotate daily by day-of-year index
